...
let {
  ...
  arrayOfAttachments
    ...
    this.loadFiles(arryOfLibraryItems).then(result => {
      ...
      this.convertAllFiles(arrayOfFilesToConvert).then(convertedFiles => {
        ...
        if (emailTemplateTo .indexOf(',') > -1){
          emailTemplateTo.split(',').map((item: any) => {
          ...
          toRecipientList.push({
            emailAddress: {address:emailTemplateTo.replace(/\s/g, "")}
          });
        }

        if (emailTemplateCC.indexOf(',') > -1){
          emailTemplateCC.split(',').map((item: any) => {
          ...
          ccRecipientsList.push({
            emailAddress: {address:emailTemplateCC.replace(/\s/g, "")}
          });
        }
          ...
          fileAttachmentsList.push({
            ...
            const mailData :IMailMessage = {
              message: {
                  ...
                  body: {
                      contentType:"Text",
                      content:emailTemplateBody==='' ? this.buildEmailBodyContent(listItem,arrayOfAttachments.join(', ')):emailTemplateBody,
                  },
  ...
  public render(): React.ReactElement<ICustomSidePanelProps> {
    ...
    const attachmentItemsAsHtml = arrayOfAttachments.map((item)=>{
      return <li>{item}</li>;
    });

  
    const bodyEmail = emailTemplateBody==='' ? this.buildEmailBodyContent(listItem,arrayOfAttachments.join(', ')):emailTemplateBody;
    ...
    return <Panel
      ...
      <DialogFooter>
        <DefaultButton disabled={buttonDisabled} text="Cancel" onClick={this._onCancel}></DefaultButton>
        <PrimaryButton disabled={buttonDisabled} text="Send" onClick={this._onSend}></PrimaryButton>
      </DialogFooter>
